...
fs.ensureFile(cli.output, function(err) {
...
function main() {
    ...
    ramlLinter.lint(cli.input, function outputFn() {
        ...
        xmlBuilder.writeAttribute('name', 'RAML Linter Suite');
        xmlBuilder.writeAttribute('tests', tests.length);
        xmlBuilder.writeAttribute('failures', errors.length);
        xmlBuilder.startElement('testcase');
        xmlBuilder.writeAttribute('name', 'Check for errors');
        ...
        fileWriter.write(xmlBuilder.toString(), function(err) {
            if (err) console.log(err);
            else console.log('Report saved to ' + cli.output);
        });
